...
class ControllerMapper : AppCompatActivity() {
    ...
    private val receiver: BroadcastReceiver = object : BroadcastReceiver() {
        ...
        override fun onReceive(context: Context, intent: Intent) {
            if (ACTION_UPDATE_CONTROLLER_MAPPER == intent.action) {
                ...
                selectedControllerPresetSpinner?.adapter = ArrayAdapter(
                    context,
                    android.R.layout.simple_spinner_dropdown_item,
                    getControllerPresetsName(context)
                )
                selectedControllerPresetSpinner?.setSelection(
                    getControllerPresetsName(context).indexOf(name)
                )
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<Toolbar>(R.id.controllerMapperToolbar).title =
            getString(R.string.controllerMapperTitle)
        ...
        if (connectedControllers.isNotEmpty()) {
            controllerConnected?.text =
                getString(R.string.connected_controller, connectedControllers[0])
        } else {
        ...
        backButton?.setOnClickListener {
            if (supportFragmentManager.backStackEntryCount > 0) {
                ...
                findViewById<Toolbar>(R.id.controllerMapperToolbar).title =
                    resources.getString(R.string.controllerMapperTitle)
            } else {
                finish()
            }
        }
        ...
        seekBarDeadZoneValue?.text = "${deadZoneSeekbar?.progress}%"
        deadZoneSeekbar?.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
        ...
        mouseSensibilitySeekBar?.progress =
            getMouseSensibility(this, preferences!!.getString(SELECTED_CONTROLLER_PRESET_KEY, "default")!!)
        mouseSensibilityValue = findViewById(R.id.mouseSensibilityValue)
        mouseSensibilityValue?.text = "${mouseSensibilitySeekBar?.progress}%"
        mouseSensibilitySeekBar?.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            @SuppressLint("SetTextI18n")
            override fun onProgressChanged(seekBar: SeekBar?, progress: Int, fromUser: Boolean) {
        ...
        selectedControllerPresetSpinner?.adapter = ArrayAdapter(
            this,
            android.R.layout.simple_spinner_dropdown_item,
            getControllerPresetsName(this)
        )
        selectedControllerPresetSpinner?.setSelection(
            getControllerPresetsName(this).indexOf(
                preferences!!.getString(SELECTED_CONTROLLER_PRESET_KEY, "default")
            )
        )
        ...
        registerReceiver(receiver, IntentFilter(ACTION_UPDATE_CONTROLLER_MAPPER))
    }
    ...
    private fun fragmentLoader(fragment: Fragment, appInit: Boolean) {
        ...
        transaction.replace(R.id.controller_mapper_content, fragment)
    ...
    companion object {
        ...
        // Mapeia as chaves para os índices usados na lista de presets
        ...
        // Função para salvar o valor do dead zone no preset atual em formato JSON
        ...
        // Função para salvar a sensibilidade do mouse
        ...
        fun getDeadZone(context: Context, name: String): Int {
            ...
            return if (index == -1) 25 else currentList[index][mappingMap[DEAD_ZONE_KEY]!!].toInt()
        }

        fun getMouseSensibility(context: Context, name: String): Int {
            val currentList = loadControllerPresets(context)
            val index = currentList.indexOfFirst { it[0] == name }
            return if (index == -1) 100 else currentList[index][mappingMap[MOUSE_SENSIBILITY_KEY]!!].toInt()
        }

        fun getMapping(context: Context, name: String, key: String): List<String> {
            val currentList = loadControllerPresets(context)
            val index = currentList.indexOfFirst { it[0] == name }
            return if (index == -1) listOf("", "") else currentList[index][mappingMap[key]!!].split(":")
        }
        ...
        fun deleteControllerPreset(context: Context, name: String) {
            ...
            if (currentList.count() == 1) {
                Toast.makeText(
                    context,
                    context.getString(R.string.removeLastPresetError),
                    Toast.LENGTH_SHORT
                ).show()
                return
            }
        ...
        // Função que carrega os presets de controle a partir do JSON armazenado no SharedPreferences.
        private fun loadControllerPresets(context: Context): MutableList<MutableList<String>> {
            ...
            return gson.fromJson(json, listType) ?: mutableListOf(
                ArrayList(Collections.nCopies(25, ":")).apply {
                    this[0] = "default"
                    this[mappingMap[DEAD_ZONE_KEY]!!] = "25"
                    this[mappingMap[MOUSE_SENSIBILITY_KEY]!!] = "100"
                }
        ...
        fun getControllerPresetsName(context: Context): List<String> {
            ...
            currentList.forEach { presetsName.add(it[0]) }
            return presetsName
        }
    }
}
